...
MAIN: while( $loop )
{
    ...
    foreach my $monitor ( <[0-9]*> )
    {
		...
		if ( $Config{ZM_SERVER_HOST} ) {
			if ( $db_monitors->{$monitor}->{ServerHost} ne $Config{ZM_SERVER_HOST} ) {
				Debug( "Skipping since it has handled by " . $db_monitors->{$monitor}->{ServerHost} . "\n" );
				next;
    ...
    while ( my ( $db_monitor, $db_events ) = each(%db_events) )
    {
